...
test_dataframe['Synthese_eval_sanit'].value_counts(normalize=True) * 100
...
test_dataframe.fillna('_', inplace=True)
...
tree_classifier = DecisionTreeClassifier(max_leaf_nodes=8)
...
params = dict(n_estimators=range(5,51,5))

cv_sets = StratifiedKFold(n_splits=10, shuffle=True, random_state=69)

grid_cv = GridSearchCV(grid_search_classifier, params, cv=cv_sets, scoring='accuracy')
...
forest_classifier = RandomForestClassifier(n_estimators=40, oob_score=True, n_jobs=-1)
